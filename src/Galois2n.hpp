...
// @target: q, r where a = qb + r, order(r) < order(b)
  ...
  template <Int od> struct GF2n {
	...
	*c = '0' + bool(plnm & mask);
	...
	if (p.plnm == 1 && i > 0) {
      ...
      cout << "\n    Define Polynomial associate lists of a and b as integers: ";
      Int a, b;
      cin >> a >> b;
      GF2n pa(a), pb(b);
  ...
  template <Int od> std::string hexseq(GF2n<od> p) {
    const char hdmarks[17] = "0123456789ABCDEF";
    Int len = (p.order + 3) / 4 + 1;
    Int plnm = p.plnm;
    char result[len];
    result[len - 1] = '\0';
    for (int i = len - 2; i >= 0; i--) {
      result[i] = hdmarks[plnm & 15];
      plnm >>= 4;
    }
